defmodule Automata do
  ...
  def filter_states(prospect_list, seen_list) do
    [prospect_head|prospect_tail] = prospect_tail
  ...
  def get_all_empty_transition_states(automata, list_of_states, seen) do
    ...
    possible_transitions = getDistincts(Map.get(current_state_transitions, :epsilon))
    filtered_transitions = getDistincts(filter_states(possible_transitions,seen))
    [filtered_transitions|get_all_transition_states(automata,:epsilon,[filtered_transitions|states_tail], [filtered_transitions|seen])]
  end



  def get_all_non_empty_transition_states(automata, input, list_of_states)

  def get_all_non_empty_transition_states(_,_,[]) do
    []
  end
  ...
  def get_all_non_empty_transition_states(automata, input, current_state) do
    transitions =  Map.get(automata, :transitions)
    current_state_transitions = Map.get(transitions, current_state)
    getDistincts(Map.get(current_state_transitions, input))
  end
  ...
  def run_nfa(automata, tape, current_state) do
    [tape_head | tape_tail] = tape
    possible_input_states = get_all_non_empty_transition_states(automata, input, current_state)
  ...
  def apply_run_nfa(automata, tape, []) do
    false
  end
  ...
  # tape = ['a','b','a']
    ...
    possible_starts = getDistincts([start_state|get_all_transition_states(automata, [start_state])
